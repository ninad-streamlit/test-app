...
def get_openai_api_key():
    ...
    # List of possible secret key names to try
    secret_key_names = ['api_key', 'OPENAI_API_KEY']
                ...
                if not api_key:
                    try:
                        if hasattr(st.secrets, key_name):
                            api_key = getattr(st.secrets, key_name, None)
                    ...
                    try:
                        if hasattr(st.secrets, '_secrets'):
                            ...
                            if isinstance(internal_secrets, dict):
                                api_key = internal_secrets.get(key_name, None)
                    except (AttributeError, TypeError):
                        pass
    ...
    # Validate and return
    if not api_key:
        raise ValueError("OPENAI_API_KEY environment variable is not set. Please add it to your .env file or Streamlit Cloud secrets as 'api_key' or 'OPENAI_API_KEY'.")
    ...
    if not api_key:
        raise ValueError("API key is empty. Please check your Streamlit Cloud secrets or .env file.")
